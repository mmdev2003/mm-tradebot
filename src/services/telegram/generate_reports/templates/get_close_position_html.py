...
def get_close_position_html(position_data):
    symbol = position_data['symbol']
    open_price = Decimal(position_data['open_price']).normalize()
    close_price = Decimal(position_data['close_price']).normalize()
    leverage = position_data['leverage']
    position_side = position_data['position_side']
    size = position_data['size']
    
    profit_in_percent = Decimal(position_data['profit_in_percent']).quantize(Decimal('0.001'), rounding=ROUND_HALF_EVEN).normalize()
    profit_in_dollars = Decimal(position_data['profit_in_dollars']).quantize(Decimal('0.001'), rounding=ROUND_HALF_EVEN).normalize()
    profit_in_percent_from_account = Decimal(position_data['profit_in_percent_from_account']).quantize(Decimal('0.001'), rounding=ROUND_HALF_EVEN).normalize()
...
>
    ...
    >
        ...
        >
            ...
            "
            >
                {profit_in_dollars}$ / {profit_in_percent}%
            </span>
        </div>
        <div
        style="
        ...
        >
            ...
            "
            >
                {profit_in_percent_from_account}%
            </span>
        </div>
        <div
        style="
