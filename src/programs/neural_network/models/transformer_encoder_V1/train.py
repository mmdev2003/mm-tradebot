...
from src.programs.neural_network.models.transformer_encoder_V1.params import params
from src.programs.neural_network.models.transformer_encoder_V1.load_dataset import TradeDatasetV1
...
def load_dataset(train_size, bs, count_prev_bars, len_seq, seed):
    dataset = TradeDatasetV1(symbol, count_prev_bars, len_seq, seed)
...
def main(n_heads, d_ff, n_layers, lr, optimizer, train, test, num_epoch, model, model_name, count_prev_bars, len_seq, bs, optimizer_name, seed):
    ...
    model_parameters_data = {
        ...
        'symbol': symbol,
        'loss': loss,
        'count_prev_bars': count_prev_bars,
        'len_seq': len_seq,
        'bs': bs,
        'lr': lr,
        'n_heads': n_heads,
...
def grid():
    seed = 1
    for count_prev_bars in params['count_prev_bars']:
        for len_seq in params['len_seq']:
            for bs in params['batch_size']:
                train, test = load_dataset(train_size, bs, count_prev_bars, len_seq, seed)
                                ...
                                for optimizer_name, optimizer in params['optimizer'].items():
                                    for num_epoch in params['num_epoch']:
                                        for model_name, model in params['predictions'].items():
                                            main(n_heads, d_ff, n_layers, lr, optimizer, train, test, num_epoch, model, model_name, count_prev_bars, len_seq, bs, optimizer_name, seed)
grid()
